...
double xEngineWorldCoord = .15;
double yEngineWorldCoord = 0.0;
double zEngineWorldCoord = 0.0;
...
void engineMarkerCallback(const visualization_msgs::MarkerArray::ConstPtr& msg)
{
	...
	if (!listener->waitForTransform ("world", foundMarker.header.frame_id, ros::Time(0), ros::Duration(.1)))
	{
		// not ready yet
		return;
	}
	...
	tf::poseMsgToTF(foundMarker.pose, enginePose);

	tf::Vector3 gripOffset(xEngineCenter, yEngineCenter, zEngineCenterFirst);

	tf::StampedTransform engineToWorld;
	listener->lookupTransform("world", foundMarker.header.frame_id, ros::Time(0), engineToWorld);
...
void publishEngine()
{
	...
	tf::Quaternion modelCorrection = tf::createQuaternionFromRPY(0, 0, 0);
	modelCorrection.normalize();

	initMarker(marker, "engine", 0, MarkerInitType::Mesh, xEngineWorldCoord, yEngineWorldCoord, zEngineWorldCoord);
...
int main(int argc, char **argv)
{
	...
	*/
    ...
    nhPrivate.param<bool>("fake", fake, false);
